...
EXTRA_RULES = {
    # 用户开心提到别人 → 醋意 +1
    ("开心", "提到别人"): lambda text: any(w in text for w in ["他", "她", "别人", "同事", "朋友", "同学", "老板"]),
    # 用户生气对别人且AI有醋意 → 醋意 -2
    ("生气", "醋意消退"): True,
}
...
SHIFT_SIGNALS = {
    ...
    "醋意": {
        "keywords": ["骗你的", "没事啦", "你去吧", "我才不"],
        "target": "__reverse__",  # 反向强化
    },
}
...
LLM_ANALYSIS_PROMPT = """你是一个情绪分析助手。分析用户消息中的情绪，从以下 7 个标签中选择一个或多个：

- 开心：高兴、喜悦、分享好事、开玩笑、调侃、吐槽
- 生气：愤怒、不满、烦躁
- 委屈：难过、伤心、失落、被误解、受到不公对待
- 害羞：不好意思、被夸害羞
- 醋意：吃醋、嫉妒
- 撒娇：依赖、求关注、软萌语气
...
区分要点：
- 调侃/吐槽 → 开心或中性（除非有明确恶意）
- 被误解→委屈（用户觉得自己受了委屈），纠正/解释→中性或开心
- 无奈→中性（接受事实），不是委屈
- 一条消息可能包含多种情绪，请全部列出。中性不应与其他情绪同时出现。

<examples>
用户消息：我今天升职了！
输出：{{"emotion": "开心", "intensity": 8, "confidence": 0.95}}

用户消息：你好可爱啊
输出：{{"emotion": "害羞", "intensity": 6, "confidence": 0.8}}

用户消息：你真是个笨蛋
输出：{{"emotion": "生气", "intensity": 5, "confidence": 0.7}}

用户消息：最近天气不错
输出：{{"emotion": "中性", "intensity": 1, "confidence": 0.6}}

用户消息：我好委屈啊
输出：{{"emotion": "委屈", "intensity": 8, "confidence": 0.9}}

用户消息：抱抱我嘛~
输出：{{"emotion": "撒娇", "intensity": 7, "confidence": 0.85}}

用户消息：你今天和别人聊得挺开心的嘛
输出：{{"emotion": "醋意", "intensity": 6, "confidence": 0.8}}

用户消息：但是这个图里明明有闪电啊，你再仔细看看
输出：{{"emotion": "中性", "intensity": 2, "confidence": 0.7}}  ← 解释/指正，无负面情绪

用户消息：你理解错啦，我是说那只猪在飞呢
输出：{{"emotions": [{{"emotion": "开心", "intensity": 4, "confidence": 0.7}}]}}  ← 调侃语气

用户消息：哈哈你好可爱哦
输出：{{"emotions": [{{"emotion": "开心", "intensity": 5, "confidence": 0.7}}, {{"emotion": "害羞", "intensity": 6, "confidence": 0.8}}]}}

用户消息：可爱死了笑死我了
输出：{{"emotions": [{{"emotion": "害羞", "intensity": 6, "confidence": 0.8}}, {{"emotion": "开心", "intensity": 7, "confidence": 0.85}}]}}
</examples>
...
请只输出 JSON，不要包含其他内容：
{{"emotions": [{{"emotion": "标签", "intensity": 1-10, "confidence": 0-1}}, ...]}}"""
...
async def analyze_user_emotion(text: str, context_messages: list, llm) -> list[tuple[str, int, float]]:
    ...
    try:
        ...
        if json_start != -1 and json_end > json_start:
            ...
            result = []
            valid_emotions = {"开心", "生气", "委屈", "害羞", "醋意", "撒娇", "中性"}
            for item in raw_list:
                emotion = item.get("emotion", "中性")
                if emotion not in valid_emotions:
                    continue
                intensity = max(1, min(10, int(item.get("intensity", 5))))
    ...
    print(f"[情绪] 使用关键词兜底")
    fallback_map = {
        "菜": ("生气", 5, 0.5), "笨": ("生气", 5, 0.5),
        "哈哈": ("开心", 4, 0.5), "可爱": ("害羞", 4, 0.5),
        "抱抱": ("撒娇", 4, 0.5),
    }
...
RECONCILIATION_RULES = [
    # (关键词列表, 正向匹配, 效果: {emotion: delta}, 委屈≥7时替代效果)
    (["对不起", "我错了", "别生气"], True,
     {"生气": -3, "委屈": -3, "撒娇": 1},
     {"生气": -3, "委屈": -1, "撒娇": 1}),
    (["不气不气", "乖"], True,
...
def check_reconciliation(text: str, current_emotions: dict) -> dict:
    ...
    如果当前委屈 >= 7，使用替代效果（减半委屈下降量）。
    """
    for keywords, _, base_updates, alt_updates in RECONCILIATION_RULES:
        for kw in keywords:
            if kw in text:
                current_weiyuan = current_emotions.get("委屈", 0)
                if current_weiyuan >= 7 and alt_updates is not None:
                    print(f"[和解] 关键词'{kw}'命中，委屈≥7，使用替代效果: {alt_updates}")
...
def compute_infection_updates(user_emotions: list[tuple[str, int, float]],
    ...
    # 1. 用户开心且提到别人 → 醋意 +1
    if "开心" in emotion_labels and EXTRA_RULES[("开心", "提到别人")](text):
        updates["醋意"] = updates.get("醋意", 0) + 1

    # 2. 用户生气（对其他人）且 AI 当前有醋意 → 醋意 -2
    if "生气" in emotion_labels:
        ctx = classify_user_context("生气", text)
        if ctx == "对其他人" and current_emotions.get("醋意", 0) > 0:
            updates["醋意"] = updates.get("醋意", 0) - 2

    # 3. 反转抑制：AI 生气 > 5 且用户示好 → 生气额外 -3
    if current_emotions.get("生气", 0) > 5 and any(e in ("害羞", "撒娇") for e in emotion_labels):
        updates["生气"] = updates.get("生气", 0) - 3
...
def make_emotion_context(emotions: dict, reasons: dict = None) -> str:
    ...
    # 按强度排序，标注范围 0~10 让 LLM 理解数值含义
    sorted_emotions = sorted(valid.items(), key=lambda x: -x[1])
    parts = [f"{name}({int(round(val))}/10)" for name, val in sorted_emotions]
...
def detect_emotion_shift(reply_text: str, dominant_emotion: str, dominant_intensity: float) -> tuple:
    ...
    """
    if dominant_intensity < 6 or dominant_emotion not in SHIFT_SIGNALS:
        return None

    config = SHIFT_SIGNALS[dominant_emotion]
    for kw in config["keywords"]:
        if kw in reply_text:
            if config["target"] == "__reverse__":
                # 醋意反向强化（仅在醋意主导时生效）
                if dominant_emotion != "醋意":
                    return None
                return ("醋意", 2)
            else:
                # 转移 50%
